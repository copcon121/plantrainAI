...
# -*- coding: utf-8 -*-
"""
Test Label Rules v3 SIMPLIFIED - Only FVG + Early Leg.

Removed:
- M5 context requirement
- pb_wave_strength_ok requirement
"""
import sys, io
...
def check_long_signal_simple(bar):
    """LONG: Bullish pullback in early leg (SIMPLIFIED)."""
    ...
    return early_leg and fvg_bull


def check_short_signal_simple(bar):
    """SHORT: Bearish pullback in early leg (SIMPLIFIED)."""
    bar_data = bar.get('bar', {})
    ...
    return early_leg and fvg_bear


print("=" * 80)
print("TESTING LABEL RULES V3 - SIMPLIFIED (FVG + EARLY LEG ONLY)")
print("=" * 80)
...
print("Rules: early_leg (1-2) AND fvg_active")
...
for jsonl_file in files:
    ...
    for raw_bar in bars:
        ...
        bar_state = {
            ...
            'bar': bar_data,
            # External CHoCH/BOS (required for leg detection)
    ...
    long_count = sum(1 for r in results if check_long_signal_simple(r))
    short_count = sum(1 for r in results if check_short_signal_simple(r))
    
    for i, bar in enumerate(results):
        if check_long_signal_simple(bar):
            all_long.append((jsonl_file.name, i, bar))
        if check_short_signal_simple(bar):
